...
function matrixFeldVon(k){ var f=k&&k.matrixFeld;
  ...
  return 'ziel'; }
var MATRIX_RANK={werkzeug:0, ziel:1, zustand:2, ablenkung:3};
function matrixRang(k){ return num(MATRIX_RANK[matrixFeldVon(k)],1); }
...
/* v2.0.0 (§7): Es gibt nur noch EINE Kette. tagesKetteDom ist ein FILTER auf
   ihr — der alte Shim (tagesKette = dfm.concat(privat)) wuerde jetzt endlos
...
ok('Gate 2.0.0: 1.13.4-Paket abgelehnt', !!imp({ appVersion:'1.13.4', karten:[{id:'x',titel:'x'}] }).fehler);
...
/* 9) §1/§2 (v1.13.1): kette ERSETZT die Kette komplett — der Auto-Nachzug
...
ok('§1 BELEG: erneuter Aufruf zieht nichts nach (kein 4 → 41)',
  JSON.stringify(tagesKetteDom('dfm'))==='["d3","d1","d4","d2"]');
/* v2.0.0 (§7): EINE Kette — 'pk1' war nicht im Paket und zaehlt jetzt
   ebenfalls als bewusst draussen (vorher lag sie in der Privat-Kette). */
ok('§1: nicht Genanntes gilt als bewusst draussen (5 in entfernt: d5-d8 + pk1)',
